...
if (!m) {
  fail(
    `cannot parse a GitHub repo from origin "${remote.stdout}" — only github.com remotes are supported`,
  );
}
...
if (numbersArg) {
  ...
  if (!list.ok) {
    fail(
      `gh issue list failed — is gh authenticated? Run "gh auth login". gh said: ${list.stderr}`,
    );
  }
...
for (const n of numbers) {
  ...
  if (!v.ok) {
    ...
    if (!pr.ok) {
      console.warn(
        `[fetch-issues] #${n} is not viewable as issue or PR; skipped (${v.stderr || pr.stderr})`,
      );
      continue;
    }
    v = pr;
...
const summary = issues
  .map(
    (i) =>
      `  #${i.number}${i.isPullRequest ? " (PR)" : ""} ${i.title}  [${i.labels.map((l) => l.name).join(", ")}]`,
  )
  .join("\n");
