...
export function ShellBlock({ shell }: { shell: ShellPart }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#303030] bg-[#151515]">
      <div className="flex items-center justify-between border-b border-[#282828] bg-[#1c1c1c] px-3 py-2">
        <div className="flex items-center gap-2 text-[12px] font-medium text-[#d5d5d5]">
        ...
        <div className={`flex items-center gap-1.5 text-[11px] ${status.className}`}>
          ...
          {typeof shell.exitCode === "number" && (
            <span className="text-[#666]">exit {shell.exitCode}</span>
          )}
        </div>
      </div>
