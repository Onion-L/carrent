...
function createChatSessionManager(
  ...
  return createProductionChatSessionManager(options);
}
  ...
  it("deletes sessions and attachments while stopping only runs owned by the thread", async () => {
    ...
    const { factory, transports } = createFakeKimiAcpTransportFactory((transport, message) => {
      if (message.method === "initialize") {
        ...
        respondAcp(transport, message, { sessionId: `session-${transports.length}` });
      }
    });
    ...
    expect(transports[0]?.sent.some((message) => message.method === "session/cancel")).toBe(true);
    expect(transports[1]?.sent.some((message) => message.method === "session/cancel")).toBe(false);
